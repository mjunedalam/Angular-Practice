...
export class PickedFormationTopsComponent {
  ...
  constructor() {
    // afterNextRender callback runs outside the injection context, so effect()
    // would throw NG0203 if called bare. Passing { injector } explicitly
    // restores the context and makes it valid.
    afterNextRender(() => {
      effect(() => {
  ...
  onGridReady(event: GridReadyEvent<PickedFormationTops>): void {
    ...
    // Load whatever is already in the store when the grid first becomes ready.
    // If wellDetails is already loaded (e.g. component lazy-rendered after data
    // arrived) this populates immediately; otherwise the effect handles it.
